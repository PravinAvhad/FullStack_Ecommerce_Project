...
export const createOrder = (order) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post(`${window.location.origin}/api/v3/order/new`, order, config);
        // console.log(data);
...
export const getmyallorders = () => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`${window.location.origin}/api/v3/myorders`);
        // console.log(data); 
...
export const getOrderDetails = (id) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`${window.location.origin}/api/v3/order/${id}`);
        // console.log(data); 
...
export const admingetallOrders = () => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`${window.location.origin}/api/v3/admin/allorders`);
...
export const updateOrder = (id, order) => async (dispatch) => {
    try {
        ...
        const config = { headers: { "Content-Type": "application/json" } };
        const { data } = await axios.put(`${window.location.origin}/api/v3/admin/order/${id}`, order, config);
...
export const Orderdelete = (id) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.delete(`${window.location.origin}/api/v3/admin/order/${id}`);
        // console.log(data);
